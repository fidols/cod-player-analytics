# COD Analytics — Plan 3: Quality Arc Page

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `pages/1_Quality_Arc.py` — Metacritic scores over time with era bands, critic vs. user score gap chart, and a personal quit-point annotation.

**Architecture:** Loads `data/games.csv` with `@st.cache_data`. Uses `era_summary` and `score_gap` from `utils/metrics.py`. Three charts: (1) line chart with era background shapes, (2) grouped bar chart critic vs. user score, (3) era summary bar. Quit point annotated as a vertical line at 2025.

**Tech Stack:** Python 3.12, Streamlit >= 1.32.0, Plotly Express/Graph Objects >= 5.18.0, pandas >= 2.0.0
...
| File | Change |
|---|---|
| `pages/1_Quality_Arc.py` | Create |

---

## Task 1: Failing test for score_gap values

**Files:**
- Modify: `tests/test_metrics.py` (append)

- [ ] **Step 1: Append two tests for era avg score per era to tests/test_metrics.py**
...
Expected: all tests pass (17 tests now including the 2 new ones — they skip if CSV is not generated yet, which is fine).
...
git commit -m "test: add Quality Arc assertion tests for era avg and score gap"
```

---

## Task 2: Build pages/1_Quality_Arc.py

**Files:**
- Create: `pages/1_Quality_Arc.py`

- [ ] **Step 1: Create pages/1_Quality_Arc.py**
...
Navigate to "Quality Arc" in the left sidebar. Expected: 3 KPI cards, line chart with era bands and quit annotation, grouped bar chart with critic vs. user scores, era average bar chart. All axis text is black.

- [ ] **Step 3: Commit**

```bash
cd /Users/mr.fidols/github/cod-player-analytics
git add pages/1_Quality_Arc.py
git commit -m "feat: add Quality Arc page — score timeline, critic vs user gap, era averages"
```
